...
// --- Macro gauge: 10Y yield, S&P 500, US dollar index (via Yahoo, keyless) ---
async function macroSeries(symbol) {
  const url =
  ...
  try {
    ...
    const closes = data?.chart?.result?.[0]?.indicators?.quote?.[0]?.close || [];
    const latest = closes.filter(v => v != null).pop();
    const prev = closes.filter(v => v != null).slice(-2)[0] ?? latest;
    if (latest == null) return null;
    const changePct = (prev != null && prev !== 0) ? ((latest - prev) / prev) * 100 : null;
    return { value: latest, changePct: changePct != null ? +changePct.toFixed(2) : null };
  } catch {
    return null;
  }
}
...
export async function getMacro() {
  const [tnx, gspc, dxy] = await Promise.allSettled([
    macroSeries('^TNX'),
    macroSeries('^GSPC'),
    macroSeries('^DXY'),
  ]);
  return {
    tenYear: tnx.status === 'fulfilled' ? tnx.value : null,
    sp500: gspc.status === 'fulfilled' ? gspc.value : null,
    dollar: dxy.status === 'fulfilled' ? dxy.value : null,
  };
}

// --- Combined market sentiment ---
export async function getMarketSentiment() {
  const [usFg, fg, vix, macro] = await Promise.allSettled([getUsFearGreed(), getFearGreed(), getVixFear(), getMacro()]);
  return {
    available: true,
