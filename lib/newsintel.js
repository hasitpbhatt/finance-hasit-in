...
export async function getNewsIntel(symbol, companyName, signal = null) {
  ...
  for (const block of googleBlocks) {
    ...
    let domain = '';
    if (sourceMatch) {
      try { domain = new URL(sourceMatch[1]).hostname.replace('www.', ''); }
      catch { /* malformed source URL from the feed — skip the domain */ }
    }
    if (title && link) {
  ...
  const avgSentiment = scored.reduce((s, a) => s + a.sentiment * a.weight, 0) / scored.reduce((s, a) => s + a.weight, 0);

  const trend = recent7.length && older7to30.length
    ? recent7.reduce((s, a) => s + a.sentiment * a.weight, 0) / recent7.reduce((s, a) => s + a.weight, 0) -
      older7to30.reduce((s, a) => s + a.sentiment * a.weight, 0) / older7to30.reduce((s, a) => s + a.weight, 0)
    : 0;
