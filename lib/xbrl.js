...
import { cachedJson, fromCache, store, runSec } from './cache.js';
...
async function fromCompact(cikPadded, concept) {
  const cached = await fromCache(compactKey(cikPadded, concept));
  if (!cached) return null;
  if (Date.now() - cached.cachedAt >= XBRL_TTL * 1000) return null;
  try { return JSON.parse(cached.text); } catch { return null; }
}

async function toCompact(cikPadded, concept, series) {
  await store(compactKey(cikPadded, concept), JSON.stringify(series), 'application/json');
}
...
async function fetchConcept(cik, concept, signal = null) {
  ...
  const results = await Promise.allSettled(
    tryTags.map(tag => runSec(() => cachedJson(
