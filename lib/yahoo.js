...
import { cachedJson, cachedText, UA, fromCache, store } from './cache.js';
...
const CRUMB_KEY = 'https://internal/yahoo-session';
...
// Obtain a Yahoo cookie + crumb. Cached at the edge (Cloudflare) or in the
// per-instance memCache (Vercel) for CRUMB_TTL, and coalesced so concurrent
// callers in one invocation share a single fetch instead of each doing their
// own fc.yahoo.com + getcrumb round-trip.
let sessionPromise = null;

async function getSession(force = false) {
  if (!force) {
    const hit = await fromCache(CRUMB_KEY);
    if (hit && hit.cachedAt && Date.now() - hit.cachedAt < CRUMB_TTL) {
      try { return JSON.parse(hit.text); } catch { /* fall through to refresh */ }
    }
  ...
  sessionPromise = (async () => {
    ...
    await store(CRUMB_KEY, JSON.stringify(session), 'application/json');
    return session;
  })();
  try {
...
function normalizeFundamental(result) {
  ...
  // Yahoo returns history in descending order, but don't rely on it: sort by
  // year (then quarter) descending so "most recent first" holds regardless.
  const qNum = (q) => {
    if (q == null) return 0;
    const m = String(q).match(/(\d+)/);
    return m ? parseInt(m[1], 10) : 0;
  };
  const orderedEHist = [...eHist].sort(
    (a, b) => (b.year ?? 0) - (a.year ?? 0) || qNum(b.quarter) - qNum(a.quarter),
  );
  const surpriseHistory = orderedEHist.slice(0, 8).map(h => ({
  ...
  // Beat streak: consecutive quarters where actual > estimate (from most recent
  // backward). computed over the newest-first ordering above.
  let beatStreak = 0;
  for (const h of orderedEHist) {
...
export function computeOptionSignals(chain, currentPrice, opts = {}) {
  ...
  // --- Max pain (true algorithm) ---
  // Max pain = the strike S that MINIMIZES total intrinsic payout to option
  // holders at expiry: Σ_calls max(0, S−Kc)·OIc + Σ_puts max(0, Kp−S)·OIp.
  // This is independent of the OI-concentration "walls" above, so it will
  // generally differ from resistance/support.
  const callLegs = calls.filter(c => c.strike != null && c.oi > 0).map(c => ({ k: c.strike, oi: c.oi }));
  const putLegs = puts.filter(p => p.strike != null && p.oi > 0).map(p => ({ k: p.strike, oi: p.oi }));
  const candidateStrikes = [];
  const seen = new Set();
  for (const leg of [...callLegs, ...putLegs]) {
    if (!seen.has(leg.k)) { seen.add(leg.k); candidateStrikes.push(leg.k); }
  }
  let maxPain = null, minPain = Infinity;
  for (const s of candidateStrikes) {
    let pain = 0;
    for (const c of callLegs) if (s > c.k) pain += (s - c.k) * c.oi;
    for (const p of putLegs) if (p.k > s) pain += (p.k - s) * p.oi;
...
export async function searchSymbols(query, max = 10, signal = null) {
